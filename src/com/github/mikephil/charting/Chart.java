...
public abstract class Chart extends View {
    ...
    // instead of drawing marker view, I prefer draw the text from each marker {
    protected void drawMarkerText() {
    	final int minNumber = Math.min(markerTextList.size(), markerColorList.size());
    	int totalHeight = 0;
    	...
    	for(int i = minNumber - 1; i >= 0; i--) {
    		...
    		mDescPaint.setColor(markerColorList.get(i));
    		Rect bounds = new Rect(); 
    		mDescPaint.getTextBounds(markerTextList.get(i), 0, 1, bounds);
    		totalHeight += bounds.height();
    		mDrawCanvas
    			.drawText(markerTextList.get(i), getWidth() - mOffsetRight - bounds.width(), 
    					getHeight() - mOffsetBottom - totalHeight, mDescPaint);
    		
    		totalHeight += bounds.height() + 5;
    	}
    }
    
    private ArrayList<String> markerTextList = new ArrayList<String>(); 
    private ArrayList<Integer> markerColorList = new ArrayList<Integer>();
    public void setMarkers(String[] text, int[] color) {
    	markerTextList.clear();
    	markerColorList.clear();
    	
    	for(int i = 0; i < text.length; i++)
    		markerTextList.add(text[i]);
    	
    	for(int i = 0; i < color.length; i++) 
    		markerColorList.add(color[i]);
    }
